...
def test_a_persons_choice_is_recorded_under_chosen_by_human_with_no_probabilities(tmp_path):
    state = decide.decision_state("causal_study", PROFILE)
    entry = decide.human_choice("causal_study", state, "treatment", OPTIONS, "treatment",
                                "the column named treatment is the intervention this synthetic generator applies",
                                as_of="2026-09-25T00:00:00+00:00", record_dir=tmp_path)
    assert entry["status"] == "OK"
    decision = entry["decision"]
    assert decision["chosen_by"] == decide.HUMAN and decision["backend"] == decide.HUMAN_BACKEND
...
def test_a_persons_choice_carrying_probabilities_or_no_reason_is_never_written(tmp_path):
    state = decide.decision_state("causal_study", PROFILE)
    entry = decide.human_choice("causal_study", state, "treatment", OPTIONS, "treatment", "because it is one")
    ...
    with pytest.raises(decide.DecisionError, match="own words"):
...
def test_a_models_decision_never_carries_a_reason_a_person_wrote(source, tmp_path):
    ...
    with pytest.raises(decide.DecisionError, match="no `why`"):
        decide.record(forged, tmp_path)


def test_a_persons_choice_can_be_linked_to_a_row_and_the_outcome_says_who_chose(tmp_path):
    state = decide.decision_state("causal_study", PROFILE)
    entry = decide.human_choice("causal_study", state, "treatment", OPTIONS, "treatment", "it is the intervention",
                                record_dir=tmp_path / "decisions")
    ...
    assert linked["outcome"]["chosen_by"] == decide.HUMAN
